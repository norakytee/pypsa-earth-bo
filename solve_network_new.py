...
def new_solar_capacity_constraint(n):
    lhs = n.generators.groupby(by='carrier')['p_nom_max'].sum()['solar']
    rhs = n.generators.groupby(by='carrier')['p_nom'].sum()['solar'] + 300
    define_constraints(n, lhs, '<=', rhs, 'Generator', 'new_solar_capacity')

def new_wind_capacity_constraint(n):
    lhs = n.generators.groupby(by='carrier')['p_nom_max'].sum()['wind']
    rhs = n.generators.groupby(by='carrier')['p_nom'].sum()['wind'] + 300
    define_constraints(n, lhs, '<=', rhs, 'Generator', 'new_wind_capacity')

def new_biomass_capacity_constraint(n):
    lhs = n.generators.groupby(by='carrier')['p_nom_max'].sum()['biomass']
    rhs = n.generators.groupby(by='carrier')['p_nom'].sum()['biomass'] + 300
